...
const input =
  'mt-1.5 w-full rounded-lg border border-ink/15 px-3 py-2.5 text-sm text-ink outline-none focus:border-spark'
const label = 'block text-xs font-semibold uppercase tracking-wider text-ink/50'
const btnSpark =
...
function LoginCard({ onLogin }: { onLogin: (u: PortalUser) => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-ink-dark px-6 py-12">
      <div className="w-full max-w-sm">
        ...
        {pending ? (
          <form onSubmit={submitTotp} className="rounded-2xl bg-white p-8 shadow-2xl">
            <h1 className="text-lg font-bold text-ink">Two-factor check</h1>
            ...
            <button
              ...
              className="mt-3 w-full text-center text-xs text-ink/40 hover:text-ink"
            >
              ← Back to sign in
            </button>
          </form>
        ) : mode === 'forgot' ? (
          <form onSubmit={submitForgot} className="rounded-2xl bg-white p-8 shadow-2xl">
            <h1 className="text-lg font-bold text-ink">Reset password</h1>
            <p className="mt-1 text-sm text-ink/50">
          ...
          <form onSubmit={submitLogin} className="rounded-2xl bg-white p-8 shadow-2xl">
...
function LeadRow({ lead, onSaved }: { lead: PortalLead; onSaved: (l: PortalLead) => void }) {
  ...
  return (
    <>
      <tr onClick={() => setOpen((o) => !o)} className="cursor-pointer border-b border-ink/5 transition hover:bg-ink/[0.02]">
        ...
        <td className="hidden max-w-xs truncate px-4 py-3 text-sm text-ink/60 lg:table-cell">{lead.message}</td>
        <td className="px-4 py-3">
          ...
          <td colSpan={6} className="px-4 py-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <p className="mt-1 whitespace-pre-wrap rounded-lg bg-white p-3 text-sm text-ink/80">{lead.message}</p>
...
function RecoveryCodes({ codes, onDone }: { codes: string[]; onDone: () => void }) {
  ...
  return (
    <div className="rounded-xl border-2 border-spark/40 bg-spark/5 p-4">
      <p className="text-sm font-bold text-ink">Save these recovery codes</p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button onClick={onDone} className="rounded-lg px-4 py-2 text-xs font-bold text-ink/50 hover:text-ink">
          I saved them
        </button>
      </div>
    </div>
    ...
    >
      ...
      {!codes && !user.totpEnabled && step === 'password' && (
        <form onSubmit={startSetup} className="max-w-xs">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button type="button" onClick={() => { setStep('idle'); setError('') }} className="px-3 text-xs font-bold text-ink/50">Cancel</button>
          </div>
        </form>
      )}
      ...
      {!codes && !user.totpEnabled && step === 'scan' && (
        <form onSubmit={enable}>
          <div className="flex flex-col items-start gap-4 sm:flex-row">
            {qr && <img src={qr} alt="2FA QR code" className="rounded-lg border border-ink/10" />}
            <div className="text-sm text-ink/70">
              <p className="font-semibold text-ink">1. Scan with your authenticator app</p>
              <p className="mt-1">Google Authenticator, Authy, 1Password, etc.</p>
              <p className="mt-3 font-semibold text-ink">Can't scan? Enter manually:</p>
              <code className="mt-1 block break-all rounded bg-ink/5 px-2 py-1 text-xs">{secret}</code>
            </div>
          </div>
      ...
      {!codes && user.totpEnabled && (
        <div>
          ...
          {showRegen && (
            <form onSubmit={regen} className="mt-4 max-w-xs rounded-xl bg-ink/[0.03] p-4">
          ...
          {showDisable && (
            <form onSubmit={disable} className="mt-4 max-w-xs rounded-xl bg-ink/[0.03] p-4">
              <label className={label}>
                Password
        ...
        {sessions.map((s, i) => (
          <li key={i} className="flex items-center justify-between gap-3 py-2.5 text-sm">
            <div className="min-w-0">
              <p className="truncate text-ink/80">{s.ua}</p>
              <p className="text-xs text-ink/40">Signed in {fmtDateTime(s.created_at)}</p>
            </div>
      ...
      <ul className="divide-y divide-ink/5">
        {activity.map((a, i) => (
          <li key={i} className="flex items-center justify-between gap-3 py-2 text-sm">
            <span className="text-ink/80">{EVENT_LABELS[a.event] || a.event}</span>
            <span className="shrink-0 text-xs text-ink/40">{fmtDateTime(a.created_at)}</span>
          </li>
        ))}
        {activity.length === 0 && !error && <li className="py-3 text-sm text-ink/40">No activity yet.</li>}
      </ul>
    </Card>
  )
}
    ...
    <Card title="Admin — reset a teammate's password" desc="Generates a one-time temporary password and signs them out everywhere.">
      ...
      {temp && (
        <div className="mt-4 rounded-xl border-2 border-spark/40 bg-spark/5 p-4">
          <p className="text-sm font-bold text-ink">Temporary password for {temp.name}</p>
          <code className="mt-2 block rounded bg-white px-3 py-2 text-center font-mono text-sm">{temp.pw}</code>
          <p className="mt-2 text-xs text-ink/60">Shown once — send it to them securely. They should change it after signing in.</p>
        </div>
      )}
    </Card>
  )
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        {view === 'security' ? (
          <>
            <h1 className="text-2xl font-bold text-ink">Security settings</h1>
            {!user.totpEnabled && (
              <p className="mt-3 rounded-lg bg-spark/10 px-4 py-3 text-sm text-ink/70">
          ...
          <>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <select value={type} onChange={(e) => setType(e.target.value)} className="rounded-lg border border-ink/15 bg-white px-3 py-2 text-sm text-ink">
                <option value="">All types</option>
                {TYPES.map((t) => (<option key={t} value={t} className="capitalize">{t}</option>))}
              </select>
              <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-lg border border-ink/15 bg-white px-3 py-2 text-sm text-ink">
